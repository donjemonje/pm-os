...
export function InviteChoices({
  ...
  const googleHref = `/api/auth/oauth/google?from=%2F&invite=${encodeURIComponent(token)}`;

  return (
    <div className="space-y-5">
      <p className="font-subtitle text-sm text-brand-muted">
        Hi {name}, you&apos;ve been invited to{" "}
        <span className="text-brand-text">{organizationName ?? "PM-OS"}</span>.
